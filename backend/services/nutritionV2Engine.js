...
import { clampNumber, parseJsonObject, formatLocalDate, normalizeFoodName, foodTriggersAllergen, expandAllergenTerms } from './nutritionUtils.js';
...
function buildFoodCatalogFilters({
  ...
  // etiquetado incompleto (p. ej. "Natto" a un alérgico a la soja).
  ...
  for (const term of allergenTerms) {
    ...
    // al gluten (la subcadena existe pero significa ausencia).
    whereClauses.push(
...
function matchesFoodFilters(food, userFoodFilters) {
  ...
  // en el catálogo ya no deja pasar un alimento peligroso.
