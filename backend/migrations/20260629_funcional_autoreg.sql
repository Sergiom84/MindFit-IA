-- Autorregulación en tiempo real para Entrenamiento Funcional (Fase 4)
-- ADITIVO y AISLADO: crea estado y función propios para Funcional, sin tocar
-- el motor de HipertrofiaV2 ni los de calistenia/crossfit/halterofilia.
--
-- Modelo (reps + RIR, idéntico a Calistenia porque Funcional es series×reps×RIR):
-- tras completar una sesión se registra el RIR medio y si se cumplió el objetivo
-- de repeticiones. La función decide el ajuste del próximo microciclo:
...
CREATE TABLE IF NOT EXISTS app.funcional_autoreg_state (
...
CREATE OR REPLACE FUNCTION app.funcional_register_session_result(
  p_user_id integer,
  p_plan_id integer,
  p_avg_rir numeric,
