-- Autorregulación Funcional: feedback SUBJETIVO como modificador acotado.
-- Mismo patrón que Calistenia (modelo RIR+objetivo). Lo OBJETIVO manda; el
-- feedback (p_subjective -1..+1) frena (no contar "fácil" si lo sintió duro) y
-- desempata (cuando es neutro). p_subjective NULL → comportamiento idéntico.

ALTER TABLE app.funcional_autoreg_state
  ADD COLUMN IF NOT EXISTS last_subjective numeric;

DROP FUNCTION IF EXISTS app.funcional_register_session_result(integer, integer, numeric, boolean);
