-- Autorregulación en tiempo real para Halterofilia (Fase 4)
-- ADITIVO y AISLADO: crea estado y función propios, sin tocar el motor de
-- HipertrofiaV2, calistenia (calistenia_autoreg_state) ni crossfit
-- (crossfit_autoreg_state).
--
-- Modelo de FUERZA (RPE + carga objetivo + técnica): tras completar una sesión
-- se registra el RPE (1-10), si se alcanzó la CARGA objetivo y si la TÉCNICA fue
-- sólida. La función decide el ajuste de carga de la próxima sesión:
...
--   - 'hold'     : mantener
--
-- Seguridad: la mala técnica NUNCA cuenta como sesión "cómoda" aunque el RPE sea
-- bajo (no se empuja carga sin técnica sólida).

CREATE TABLE IF NOT EXISTS app.halterofilia_autoreg_state (
...
CREATE OR REPLACE FUNCTION app.halterofilia_register_session_result(
  p_user_id integer,
  p_plan_id integer,
  p_rpe numeric,
