-- Autorregulación Halterofilia: feedback SUBJETIVO como modificador acotado.
-- Modelo de FUERZA (RPE + carga + técnica). Lo OBJETIVO manda; el feedback
-- (p_subjective -1..+1) frena ("fácil" + "me costó" → no cuenta como cómoda) y
-- desempata (neutro). La técnica deficiente sigue siendo 'hard' siempre (el
-- subjetivo NO puede forzar progreso sin técnica). NULL → idéntico al anterior.

ALTER TABLE app.halterofilia_autoreg_state
  ADD COLUMN IF NOT EXISTS last_subjective numeric;

DROP FUNCTION IF EXISTS app.halterofilia_register_session_result(integer, integer, numeric, boolean, boolean);
