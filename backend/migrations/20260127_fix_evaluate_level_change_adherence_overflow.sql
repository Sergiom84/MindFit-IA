-- Evita overflow en adherencia cuando hay muchas sesiones en 30 días (p.ej. simulaciones)
...
BEGIN
  ...
  -- Sesiones esperadas por nivel (30 días)
  v_expected_sessions := CASE
    WHEN v_current_level = 'Principiante' THEN 12::NUMERIC -- 3 sesiones/semana
    WHEN v_current_level = 'Intermedio' THEN 16::NUMERIC -- 4 sesiones/semana
    ELSE 20::NUMERIC -- 5 sesiones/semana
  END;

  -- Calcular adherencia y acotarla a [0, 100] para evitar overflows
  v_adherence := CASE
    WHEN v_expected_sessions > 0 THEN
      (v_sessions_last_month::NUMERIC / v_expected_sessions) * 100
    ELSE 0::NUMERIC
  END;

  v_adherence := LEAST(100::NUMERIC, GREATEST(0::NUMERIC, COALESCE(v_adherence, 0::NUMERIC)));
