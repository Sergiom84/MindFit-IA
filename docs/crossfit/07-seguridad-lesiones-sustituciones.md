...
Actualmente `limitaciones_fisicas` canonico se reutiliza y el filtro regex evita zonas amplias. En rama futura, la regex es fallback de confianza baja: solo puede endurecer/bloquear, no autorizar un movimiento. Equipamiento se consulta de inventario; sueño/estres/fatiga se capturan por check-in temporal y no se guardan como atributo fijo del perfil.
