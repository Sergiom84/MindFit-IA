...
Estado: `RECONSTRUCCION_LIMPIA_QA_PENDIENTE`
...
para principiante, intermedio y avanzado. El desarrollo original quedó validado
en `codex/crossfit-profesional-v2@9c5cd4f`. Para excluir completamente la
historia contaminada de PR #63, el mismo árbol funcional se reconstruye en
`codex/crossfit-profesional-v2-clean` desde `origin/main@c233ceb`, que incluye
Fase 0, CAL-00, la identidad canónica de Hipertrofia y el saneamiento de PR #67.
PR #63 ejecutó en
...
26/26 integración PostgreSQL/RLS y 16/16 E2E desktop/móvil. PR #67 saneó `main`
y obtuvo GitGuardian verde sin bypass; el PR sustituto debe repetir equivalencia,
CI y GitGuardian antes de autorizar ready/merge. Tampoco se autorizan
...
5. `HISTORIA_LIMPIA_RECONSTRUIDA`: la cuenta/BD histórica fue eliminada según
   confirmación del propietario; PR #67 saneó `main` y GitGuardian pasó. PR #63
   no se fusionará.
6. `REQUIERE_VALIDACION_HUMANA_PREPRODUCCION`: entrenador, nutricionista y
   profesional clínico revisan; legal decide el nombre.
7. El PR sustituto solo pasa a ready y merge protegido cuando demuestre
   equivalencia, CI integral y GitGuardian verde. Nunca push directo a `main`.
...
autorización de migraciones/flags y el recheck de la historia limpia previo al
merge. La evidencia específica de v2 está detallada en
