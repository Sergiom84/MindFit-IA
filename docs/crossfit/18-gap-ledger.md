...
| GitGuardian                          | PR #67 saneó `main`; 3 commits escaneados sin secretos; rama limpia desde `c233ceb`     | PENDIENTE_RECHECK_PR_SUSTITUTO                |
...
- `FASE_0_COMPARTIDA_DESBLOQUEADA_PARA_DESARROLLO`: contratos disponibles
  desde `e7f5711`; rama limpia reconstruida desde `origin/main@c233ceb`.
...
- `HISTORIA_LIMPIA_RECONSTRUIDA`: el propietario confirmó que la cuenta/BD
  histórica fue eliminada; PR #67 saneó `main` y GitGuardian pasó sin bypass.
  PR #63 no se fusionará y el PR sustituto debe obtener su propio check verde.
- `IMPLEMENTACION_EN_RAMA`: código, flags y tests v2 en
  `codex/crossfit-profesional-v2-clean`.
...
eficacia deportiva/clínica, el shadow de rollout ni la activación productiva. La
historia contaminada de PR #63 no se fusionará. La rama sustituta solo será apta
para ready/merge tras equivalencia, QA integral y GitGuardian verdes; aun
entonces no será apta para producción sin los gates externos.
