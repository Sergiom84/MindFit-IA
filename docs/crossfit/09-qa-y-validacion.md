...
productiva. En PR #63, GitGuardian quedó rojo por una credencial histórica real;
fue un gate de seguridad de merge, no un fallo funcional ocultable.

Ese resultado pertenece a PR #63. El propietario confirmó que la cuenta y la BD
históricas fueron eliminadas; PR #67 saneó `main` y obtuvo GitGuardian verde sin
bypass. La rama sustituta se construye desde ese `main` y debe repetir todos los
gates, incluido GitGuardian, antes de considerarse fusionable.
...
Antes de activación productiva: entrenador cualificado revisa umbrales,
progresiones, 120 mappings y safety matrix; nutricionista deportivo revisa
energia/macros/timing/RED-S. La muestra mínima es 30 planes (10/nivel), >=90
sesiones, 15 sustituciones y todos los perfiles de alto riesgo. Gate: cero
finding critico/alto sin resolver y >=95 % acuerdo sobre
stimulus/clasificacion; desacuerdo restante documentado. Embarazo/posparto
requiere profesional competente especifico.

## Checklist de salida

- [x] Fase 0 compartida desbloqueada para desarrollo y branch aislada creada.
- [x] Catalogo versionado, validado, RLS y rollback técnico en BD aislada; no aplicado en producción.
...
- [x] Nutrición CrossFit idempotente en shadow/BD aislada.
- [x] E2E movil/escritorio/offline verde en el cierre histórico; repetir en PR sustituto.
- [x] Regresion ajena verde sin modificar otras metodologias; repetir en PR sustituto.
- [x] Observabilidad y alertas probadas a nivel unitario; pendiente muestra QA/operativa.
- [ ] PR sustituto con equivalencia, CI y GitGuardian verdes.
